...
# -*- coding: utf-8 -*-
"""
GM SDK API Demo: get_next_n_trading_dates
获取后N个交易日
...
OUTPUT_DIR = 'output/next_trading_dates_data'
            ...
            try:
                ...
                if date_objects:
                    ...
                    analysis['date_span_analysis'] = {
                        'days_from_reference_to_earliest': (earliest_date - ref_date_obj).days,
                        'days_from_reference_to_latest': (latest_date - ref_date_obj).days,
...
def test_get_next_n_trading_dates():
    """测试get_next_n_trading_dates API"""
    print("=" * 60)
    print("测试 get_next_n_trading_dates API")
    print("=" * 60)
    ...
    for i, case in enumerate(all_cases, 1):
        ...
        try:
            ...
            # 调用API
            result = get_next_n_trading_dates(**case['params'])
            ...
            # 保存结果
            if len(dates_list) > 0:
                filename_prefix = f"next_trading_dates_case_{i}_{case['name'].replace(' ', '_')}"
            ...
            filename_prefix = f"next_trading_dates_error_case_{i}"
            save_results([], error_analysis, filename_prefix)
        
        print("-" * 40)
    
    print("\n" + "=" * 60)
    print("get_next_n_trading_dates API 测试完成")
    print("=" * 60)

if __name__ == '__main__':
    test_get_next_n_trading_dates()