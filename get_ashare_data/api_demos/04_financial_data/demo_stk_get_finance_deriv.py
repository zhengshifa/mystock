...
# -*- coding: utf-8 -*-
"""
GM SDK API Demo: stk_get_finance_deriv
获取股票衍生财务指标
...
OUTPUT_DIR = 'output/finance_deriv_data'
...
def convert_finance_deriv_to_dict(deriv_obj):
    """将衍生财务指标对象转换为字典"""
    if not deriv_obj:
        return {}
    ...
    # 获取对象的所有属性
    for attr in dir(deriv_obj):
        if not attr.startswith('_'):
            try:
                value = getattr(deriv_obj, attr)
                if not callable(value):
                    deriv_dict[attr] = value
            except:
                pass
    
    return deriv_dict

def analyze_finance_deriv(deriv_list, symbols, start_date, end_date):
    """分析衍生财务指标数据"""
    if not deriv_list:
        return {}
    ...
    if deriv_list:
        ...
        for deriv in deriv_list:
            deriv_dict = convert_finance_deriv_to_dict(deriv)
            if deriv_dict:
                deriv_dicts.append(deriv_dict)
        
        analysis['deriv_data'] = deriv_dicts
        ...
        for deriv_dict in deriv_dicts:
            # 股票统计
            symbol = deriv_dict.get('symbol', 'Unknown')
            symbol_stats[symbol] = symbol_stats.get(symbol, 0) + 1
            
            # 报告期统计
            pub_date = deriv_dict.get('pub_date', None) or deriv_dict.get('report_date', None)
            if pub_date:
        ...
        all_fields = set()
        for deriv_dict in deriv_dicts:
            all_fields.update(deriv_dict.keys())
...
def save_results(deriv_list, analysis, filename_prefix):
    """保存结果到文件"""
    ...
    if deriv_list:
        ...
        if deriv_dicts:
            ...
            with open(json_file, 'w', encoding='utf-8') as f:
                json.dump(deriv_dicts, f, ensure_ascii=False, indent=2, default=str)
            print(f"数据已保存到: {json_file}")
            
            # CSV格式
            csv_file = os.path.join(OUTPUT_DIR, f'{filename_prefix}_data_{timestamp}.csv')
            df = pd.DataFrame(deriv_dicts)
...
def test_stk_get_finance_deriv():
    """测试stk_get_finance_deriv API"""
    print("=" * 60)
    print("测试 stk_get_finance_deriv API")
    print("=" * 60)
    ...
    for i, case in enumerate(all_cases, 1):
        ...
        try:
            ...
            # 调用API
            result = stk_get_finance_deriv(**case['params'])
            ...
            # 分析数据
            analysis = analyze_finance_deriv(
                deriv_list, 
            ...
            if len(deriv_list) > 0:
                print(f"  - 数据示例（前3条）:")
                for j, deriv in enumerate(deriv_list[:3]):
                    deriv_dict = convert_finance_deriv_to_dict(deriv)
                    symbol = deriv_dict.get('symbol', 'N/A')
                    pub_date = deriv_dict.get('pub_date', 'N/A')
                    print(f"    {j+1}. {symbol} - {pub_date}")
                if len(deriv_list) > 3:
                    print(f"    ... 还有 {len(deriv_list) - 3} 条记录")
            
            # 保存结果
            if len(deriv_list) > 0:
                filename_prefix = f"finance_deriv_case_{i}_{case['name'].replace(' ', '_')}"
                save_results(deriv_list, analysis, filename_prefix)
            ...
            filename_prefix = f"finance_deriv_error_case_{i}"
            save_results([], error_analysis, filename_prefix)
        
        print("-" * 40)
    
    print("\n" + "=" * 60)
    print("stk_get_finance_deriv API 测试完成")
    print("=" * 60)

if __name__ == '__main__':
    test_stk_get_finance_deriv()